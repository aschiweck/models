...
color("tomato") {
    gridfinityInit(gridx, gridy, height(gridz, gridz_define, enable_lip, enable_zsnap), height_internal, length) {
        cut(x=0, y=0, w=1, h=1, t=5, s=true);
        cut(x=1, y=0, w=1, h=1, t=5, s=true);
        cut(x=2, y=0, w=1, h=2, t=5, s=false);

        cut_move(x=0, y=1, w=0.5, h=3) {
          hull() {
            translate([0,83.0 / 2 - 1.5,0]) cylinder(h=13+10, d=4.5, center=true);
            translate([0,-83.0 / 2 + 1.5,0]) cylinder(h=13+10, d=4.5, center=true);
          }

          hull() {
            translate([0,50,0]) cylinder(h=13, d=15, center=true);
            translate([0,-50,0]) cylinder(h=13, d=15, center=true);
          }
          translate([0,0,-4]) sphere(d=15, true);
        }

        cut(x=0.5, y=1, w=1.5, h=1, t=5, s=false);
        cut(x=0.5, y=2, w=0.75, h=1, t=5, s=false);
        cut(x=1.25, y=2, w=0.75, h=1, t=5, s=false);
        cut(x=0.5, y=3, w=0.75, h=1, t=5, s=false);
        cut(x=1.25, y=3, w=0.75, h=1, t=5, s=false);
    }
