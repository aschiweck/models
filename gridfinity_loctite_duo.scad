...
module round_hole_with_fillet (radius, height=42, fillet=1.0) {
  $radius=10.5;
  $height=42;
  $fillet=1.0;
  $fn=100;
  ...
  translate([0,0,-height/2])
    rotate([180,0,0]){
      translate([0,0,-height/2]) {
        rotate_extrude()
            difference() {
              square([fillet+radius, fillet]);
              translate([fillet+radius, fillet])
                circle(r=fillet);
            };
...
color("tomato") {
    ...
    gridfinityInit(gridx, gridy, height(gridz, gridz_define, enable_lip, enable_zsnap), height_internal, length) {
        ...
        cut_move(x=2, y=1, w=1, h=2) {
            round_hole_with_fillet(radius=10.5);
            // cylinder(r=10.5, h=100, center=true);
        }
        cut_move(x=2, y=5, w=1, h=2) {
            round_hole_with_fillet(radius=14.25);
            // cylinder(r=14.25, h=100, center=true);
        }
    }
